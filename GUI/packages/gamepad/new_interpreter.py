...
def end_point(xDot = 0.0, yDot = 0.0, zDot = 0.0):
	...
	# test 1
	# a = (-L1 / 2)* math.sin(t1 + t2) - (L1 / 2) * math.sin(t1 - t2) - L2 * math.sin(t1 + t4)
	# b = (-L1 / 2) * math.sin(t1 + t2) + (L1 / 2) * math.sin(t1 - t2)
	# c = -L2 * math.sin(t1 + t4)
	# d = (L1 / 2) * math.cos(t1 + t2) + (L1 / 2) * math.sin(t1 - t2) + L2 * math.sin(t1 + t4)
	# e = (L1 /2) * math.cos(t1 + t2) - (L1 /2) * math.cos(t1 - t2)
	# f = L2 * math.cos(t1 + t4)
	# g = -L1 * math.cos(t1 + t4)
